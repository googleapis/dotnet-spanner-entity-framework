...
namespace Google.Cloud.EntityFrameworkCore.Spanner.Storage.Internal
{
    ...
    {
        private readonly SpannerCommand _request;
        private readonly long _updateCount;

        internal RetriableDmlStatement(SpannerCommand request, long updateCount)
        {
        ...
        async Task IRetriableStatement.Retry(SpannerRetriableTransaction transaction, CancellationToken cancellationToken, int timeoutSeconds)
        {
            try
            {
                ...
                if (await _request.ExecuteNonQueryAsync(cancellationToken).ConfigureAwait(false) != _updateCount)
                {
    ...
    internal sealed class FailedDmlStatement : IRetriableStatement
    {
        ...
        internal FailedDmlStatement(SpannerCommand request, SpannerException exception)
        {
            this._request = request;
        ...
        async Task IRetriableStatement.Retry(SpannerRetriableTransaction transaction, CancellationToken cancellationToken, int timeoutSeconds)
        {
            ...
            {
                _request.Transaction = transaction;
                await _request.ExecuteNonQueryAsync(cancellationToken).ConfigureAwait(false);
    ...
    public class RetriableSpannerBatchCommand
    {
        private readonly IList<SpannerCommand> _commands = new List<SpannerCommand>();
        internal SpannerRetriableTransaction Transaction { get; set; }
    ...
    {
        private readonly RetriableSpannerBatchCommand _request;
        private readonly IEnumerable<long> _updateCounts;

        internal RetriableBatchDmlStatement(RetriableSpannerBatchCommand request, IEnumerable<long> updateCounts)
        {
            this._request = request;
        ...
        async Task IRetriableStatement.Retry(SpannerRetriableTransaction transaction, CancellationToken cancellationToken, int timeoutSeconds)
        {
            ...
            {
                _request.Transaction = transaction;
                if (!_updateCounts.SequenceEqual(await _request.CreateSpannerBatchCommand().ExecuteNonQueryAsync(cancellationToken).ConfigureAwait(false)))
                {
                    throw new SpannerAbortedDueToConcurrentModificationException();
                }
            }
    ...
    internal sealed class FailedBatchDmlStatement : IRetriableStatement
    {
        ...
        internal FailedBatchDmlStatement(RetriableSpannerBatchCommand request, SpannerException exception)
        {
            this._request = request;
            this._exception = exception;
        }
        ...
        async Task IRetriableStatement.Retry(SpannerRetriableTransaction transaction, CancellationToken cancellationToken, int timeoutSeconds)
        {
            ...
            {
                _request.Transaction = transaction;
                await _request.CreateSpannerBatchCommand().ExecuteNonQueryAsync(cancellationToken).ConfigureAwait(false);
                // Fallthrough and throw the exception at the end of the method.
            }
    ...
    public sealed class SpannerRetriableTransaction : DbTransaction
    {
        ...
        private SpannerTransaction _spannerTransaction;
        ...
        internal async Task RetryAsync(CancellationToken cancellationToken, int timeoutSeconds = MAX_TIMEOUT_SECONDS)
        {
            ...
            {
                _retryCount++;
                _spannerTransaction = await _connection.BeginTransactionAsync(cancellationToken);
                try
                {
        ...
        public async Task<DateTime> CommitAsync(CancellationToken cancellationToken = default)
        {
            while (true)
            {
                try
                {
                    return await _spannerTransaction.CommitAsync(cancellationToken).ConfigureAwait(false);
                }
                catch (SpannerException e) when (e.ErrorCode == ErrorCode.Aborted)
                {
        ...
        async Task<long> ExecuteDmlAsync(ExecuteSqlRequest request, CancellationToken cancellationToken, int timeoutSeconds)
        {
            while (true)
            {
                try
                {
                    long res = await _spannerTransaction.ExecuteDmlAsync(request, cancellationToken, timeoutSeconds).ConfigureAwait(false);
        ...
        public SpannerBatchCommand CreateBatchDmlCommand() => _spannerTransaction.CreateBatchDmlCommand();

        async Task<IEnumerable<long>> ExecuteBatchDmlAsync(ExecuteBatchDmlRequest request, CancellationToken cancellationToken, int timeoutSeconds)
        {
            while (true)
            {
                try
                {
                    var res = await _spannerTransaction.ExecuteBatchDmlAsync(request, cancellationToken, timeoutSeconds).ConfigureAwait(false);
